  ...
  return (
    <header className="relative min-h-screen flex items-center justify-center overflow-hidden bg-black text-white">
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-8 lg:px-12 mb-16 md:mb-24 lg:mb-32 h-full flex flex-col justify-end items-start mt-auto max-w-screen-2xl">
        <h1 className="font-headline text-4xl md:text-6xl lg:text-7xl xl:text-8xl mb-6 md:mb-8 leading-[1.1] tracking-tight max-w-5xl text-left">
          Jūsų poilsio zona kieme.
        </h1>
        <div className="flex flex-col md:flex-row items-start justify-start gap-4 md:gap-6">
          <Link href={ROUTES.CATALOG}>
            <button className="w-full md:w-auto px-6 py-3 md:px-8 md:py-4 lg:px-10 lg:py-5 bg-white text-black text-xs md:text-sm font-bold tracking-[0.2em] uppercase transition-transform hover:scale-105 active:opacity-70">
              RINKTIS MODELĮ
            </button>
          </Link>
          <Link href={ROUTES.CONTACT}>
            <button className="w-full md:w-auto px-6 py-3 md:px-8 md:py-4 lg:px-10 lg:py-5 border border-white text-white text-xs md:text-sm font-bold tracking-[0.2em] uppercase transition-colors hover:bg-white/10 active:opacity-70">
              15 MIN. KONSULTACIJA
            </button>
          </Link>
        </div>
...
function TrustStrip() {
  return (
    <div className="absolute bottom-0 w-full bg-black py-6 md:py-8 lg:py-10 border-t border-white/10">
